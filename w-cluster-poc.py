...
try: SELECTED_TXT                            # profiler 가 만든 추천 칼럼 파일. 있으면 이걸 우선 사용
...
# 사용할 fail type 선택: selected_columns.txt 가 있으면 그걸 우선 (profiler 결과)
use_types = None
if SELECTED_TXT and Path(SELECTED_TXT).exists():
    want = [ln.strip() for ln in Path(SELECTED_TXT).read_text(encoding="utf-8").splitlines() if ln.strip()]
    fset = {c.lower(): c for c in fail_cols}
    use_types = [fset[w.lower()] for w in want if w.lower() in fset]
    miss = [w for w in want if w.lower() not in fset]
    print(f"selected_columns.txt: {len(use_types)} columns matched"
          + (f", {len(miss)} not found (e.g. {miss[:3]})" if miss else ""))
if not use_types:
    totals = df[fail_cols].sum().sort_values(ascending=False)
    use_types = list(totals.head(TOP_FAIL_TYPES).index)
    print(f"using top {len(use_types)} fail types by total (selected_columns.txt 없음)")
